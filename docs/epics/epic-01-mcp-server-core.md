...
- **Acceptance Criteria:**
  - [ ] `mcp-server/server.py` exists and is a valid MCP server using the `mcp` Python SDK
  - [ ] Server reads `~/.claude/specumentation-mcp/projects.json` for registered projects
  - [ ] Server detects active project from working directory on startup
  - [ ] If no active project is detected, server returns a helpful error message
...
- **Acceptance Criteria:**
  - [ ] `list_projects()` returns all entries from `projects.json` with path and name
  - [ ] `set_project(path)` validates the target path has a `docs/concept/` directory
  - [ ] `set_project(path)` switches the server's active project context
  - [ ] `set_project()` returns an error if path is invalid or not a specumentation project
- **Files:** `mcp-server/server.py`
...
- **Acceptance Criteria:**
  - [ ] `get_overview()` returns the full content of `docs/concept/00-overview.md`
  - [ ] `list_concepts()` parses the Concept Index table and returns structured data (number, name, status, last updated)
  - [ ] Both tools return an error if no active project is set
- **Files:** `mcp-server/server.py`
...
- **Acceptance Criteria:**
  - [ ] `get_concept(topic)` finds and returns the matching concept file (by slug or number prefix)
  - [ ] `get_concept()` returns an error if the concept is not found
  - [ ] `update_concept(topic, content)` writes the full content to the concept file
  - [ ] `update_concept()` preserves Change Log entries from the original file
  - [ ] Both tools return an error if no active project is set
- **Files:** `mcp-server/server.py`
